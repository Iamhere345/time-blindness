...
func _ready() -> void:
	for i in range(4):
		var lhs = randi_range(1, 10)
		var rhs = randi_range(1, 10)
		var ans = [lhs + rhs, lhs + rhs - randi_range(-5, 5)].pick_random()
		...
		var label: Label = question.get_child(2)
		
		label.text = "%s + %s = %s" % [lhs, rhs, ans]
